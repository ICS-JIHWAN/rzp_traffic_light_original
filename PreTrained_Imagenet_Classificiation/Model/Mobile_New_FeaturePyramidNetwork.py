...
class DepthWise_ConvBlock(nn.Module):
    def __init__(self, nin, nou, kernel_size, stride, padding):
        super(DepthWise_ConvBlock, self).__init__()
        self.depthwise = nn.Sequential(
            nn.Conv2d(nin, nou, kernel_size=kernel_size, stride=stride, padding=padding, groups=nin),
            nn.BatchNorm2d(nou),
            nn.ReLU6()
        )
...
class SeparateConv2d(nn.Module):
    def __init__(self, nin, nou, kernel_size, stride, padding):
        ...
        self.stride = stride
        #
        self.mobilev2 = nn.Sequential(
            ConvBlock(nin, nin * expansion, kernel_size=1, activation=True),
            DepthWise_ConvBlock(nin * expansion, nin * expansion, kernel_size, stride, padding),
            ConvBlock(nin * expansion, nou, kernel_size=1, activation=False)
        )

        if nin != nou:
            self.proj = nn.Sequential(
                nn.Conv2d(nin, nou, kernel_size=1, stride=stride),
                nn.BatchNorm2d(nou)
            )
        else:
            self.proj = nn.Sequential()
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.residual = nn.Sequential(
            SeparateConv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1)
        )
...
class FPN(nn.Module):
    ...
    def __init__(self, layers, bottle=False):
        ...
        self.expansion = 4
        self.layer2 = self._make_residual_block(64, 128, layers[0])
        self.layer3 = self._make_residual_block(128, 256, layers[1], stride=2)
        self.layer4 = self._make_residual_block(256, 512, layers[2], stride=2)
        self.layer5 = self._make_residual_block(512, 768, layers[3], stride=2)

        fpn_sizes = [self.layer3[layers[1] - 1].residual[-1].mobilev2[-1].conv[0].out_channels,
                     self.layer4[layers[2] - 1].residual[-1].mobilev2[-1].conv[0].out_channels,
                     self.layer5[layers[3] - 1].residual[-1].mobilev2[-1].conv[0].out_channels]
    ...
    def _make_residual_block(self, in_planes, out_planes, num_block, stride=1):
        blocks = [ResidualBlock(in_planes, in_planes)]
        for i in range(1, num_block - 1):
            blocks.append(ResidualBlock(in_planes, in_planes))
        blocks.append(ResidualBlock(in_planes, out_planes, stride))
...
class Classification(nn.Module):
    def __init__(self, num_classes=1000):
        #   P7_x(c:256) -> (conv3x3) -> P7_y(c:n_c*2) -> (bn+relu) -
        #   P6_x(c:256) -> (conv3x3) -> P6_y(c:n_c*2) -> (bn+relu)  |
        #   P5_x(c:256) -> (conv3x3) -> P5_y(c:n_c*2) -> (bn+relu)  |-> sum(GAP) -> (FCL)
        #   P4_x(c:256) -> (conv3x3) -> P4_y(c:n_c*2) -> (bn+relu)  |
        #   P3_x(c:256) -> (conv3x3) -> P3_y(c:n_c*2) -> (bn+relu) _|
    ...
    def cal_accuracy(self, prediction, target):
        softmax = nn.Softmax()
        prediction_classes = softmax(prediction)
        prediction_classes = prediction_classes.argmax(dim=1)
...
if __name__ == '__main__':
    ...
    backbone = FPN(layers=layer_dict['resnet34'], bottle=True)
